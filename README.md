...
`TimeResolution` 默认使用 detector 配置中恰好三个 `role: "Tracker"` 的探测器，也可以在脚本配置中用 `"trackerIDs": [id1, id2, id3]` 明确指定。每个探测器使用 `XYMean` 时间，即与该 track 匹配的 X/Y clusters 所包含有效读出通道拟合时间的逆方差加权平均值；Pad DUT 同样使用匹配 cluster 内有效通道拟合时间按 `1/timeError²` 加权。没有任何有效正误差时才退回普通平均。`timingWaveform` 可显式配置时间拟合的 `mode`、`cfdFraction` 等 `HitProcessor` 参数。三组时间差只使用三个 tracker 时间都有效的共同事件样本，以单高斯拟合宽度解出各层分辨率，再按 `1/sigma_i^2` 构造加权 `trackTime`。`TimeResolution.root/TrackTimes` 保存三层时间、权重和加权 track 时间；启用 `analyzeDUTTiming` 后，`DUTTimes` 和 `DUTTimeResolution/` 保存 DUT 时间差及扣除 track reference 后的 DUT 分辨率。`DUTTimeResolution/DUT_<id>/Correlations/` 保存 DUT-track residual 相对于最大/平均幅度、cluster charge、size、centroid、local X/Y 及 track predicted X/Y 的二维分布、profile 和 Pearson 系数。若输出目录存在 `OscilloscopeAnalysis.root`，脚本会读取 `Events.referenceTime`，并从三张时间差直方图保存的高斯拟合恢复 reference-time 分辨率；也可通过 `oscilloscopeFile` 指定其他文件，设为空字符串则禁用。`OscilloscopeT0/`、`EventTimes`、`TrackTimes` 和 `DUTTimes` 保存匹配结果，`ResolutionSummary.resolutionNs` 保存从 detector/track/DUT 与 reference time 的拟合宽度中扣除 reference-time 方差后的时间分辨率。`OscilloscopeT0/TimingEfficiency/` 只对可用 DUT 时间扫描固定宽度窗口。分母参考 `DUTEfficiencyAnalysis` 的口径，要求同时具有示波器 T0，且 track 预测交点位于 pad 有效面积内。分子为其中 residual 落入窗口的事例数；没有空间匹配命中、没有有效时间或落在窗外的事例只计入对应分母。默认使用 25 ns 窗口和 0.5 ns 扫描步长，可由 `timingEfficiencyWindowNs`、`timingEfficiencyStepNs` 覆盖。扫描图横轴是 25 ns 窗口的起点，因此最高点横坐标与标注区间左端一致；扫描范围覆盖 residual 中央 99% 所允许的完整窗口平移范围。每个来源只输出效率扫描画布 `c_*` 和 residual 画布 `c_residual_*`；最高点以红色星标和竖线标出，residual 图显示中央 99% 数据并完整包含最佳窗口，同时叠加最佳窗口的红色虚线边界和高斯拟合。
