...
`TimeResolution` 默认使用 detector 配置中恰好三个 `role: "Tracker"` 的探测器，也可以在脚本配置中用 `"trackerIDs": [id1, id2, id3]` 明确指定。每个 Tracker 使用 `XYMean` 时间，即与该 track 匹配的 X/Y clusters 所包含有效读出通道拟合时间的逆方差加权平均值；Pad DUT 同样使用匹配 cluster 内有效通道拟合时间按 `1/timeError²` 加权。没有任何有效正误差时才退回普通平均。`timingWaveform` 可显式配置时间拟合的 `mode`、`cfdFraction` 等 `HitProcessor` 参数。

`TimeResolution.root/RawTimeDistributions/` 保存每个探测器的原始 cluster time 和加权 `trackTime` 分布。`WithoutExternalT0/TrackerPairDifferences/` 只使用三个 Tracker 时间都有效的共同事件，保存三组两两时间差直方图并以单高斯拟合宽度解出各 Tracker 分辨率，再按 `1/sigma_i²` 构造 `trackTime`；`WithoutExternalT0/DUT/DUT<id>/hDUTMinusTrackTime` 保存 DUT-track residual，DUT 本征分辨率在扣除 track-time 分辨率后输出到终端。

若输出目录存在 `OscilloscopeAnalysis.root`，脚本读取 `Events.referenceTime`；也可通过 `oscilloscopeFile` 指定其他文件，设为空字符串则禁用。外部 T0 作为理想参考，不扣除其自身分辨率。`WithExternalT0/<detector>/hClusterTimeMinusT0` 保存所有 Tracker/DUT 的 cluster-time residual，`WithExternalT0/TrackTime/hTrackTimeMinusT0` 保存 track-time residual，拟合宽度直接作为对应时间分辨率。`WithExternalT0/DUTTimingEfficiency/DUT<id>/` 仅对 DUT 保存 `hTimeResidual` 和 `hWindowEfficiency25ns`。效率分母要求同时具有外部 T0，且 track 预测交点位于 DUT 有效面积内；没有空间匹配、没有有效 DUT 时间或落在窗口外的事例只计入分母。默认窗口宽度为 25 ns、扫描步长为 0.5 ns，可由 `timingEfficiencyWindowNs` 和 `timingEfficiencyStepNs` 覆盖。所有残差直方图直接附带高斯拟合函数，不另外保存 Canvas。主目录的 `TimingTree` 保存逐 track 的三个 Tracker、加权 track、可选 DUT 和 external T0 时间；`TimingResults` 保存拟合、分辨率及 DUT 时间窗效率结果。

`OscilloscopeAnalysis` 独立分析 LeCroy 分段 CSV。未配置目录时自动读取 `<base>/scope/<runID>/` 下的 C1–C4 默认文件名：C4 先用短窗口中值滤波抑制高频毛刺，再在 `eventIDTimeWindowNs`（默认 `[-100,0] ns`）内选择第一个下降沿，并按负逻辑串行帧 `start(1) + 16-bit eventID + stop(0)` 解码，位周期为 25 ns；`eventIDMedianFilterSamples` 控制中值滤波宽度，默认 5 且必须为正奇数。C1–C3 不做波形滤波，以 `signalTimeWindowNs`（默认 `[-160,-80] ns`，相对 `eventIDTime`）之前样本的中位数作为 baseline，只在窗内寻找负脉冲最低点，并用该脉冲自身幅度计算 CFD 阈值；时间取最低点之前最近的下降 crossing。只有 eventID 和三路时间都有效的完整事件会写入默认输出 `<base>/result/<runID>/OscilloscopeAnalysis.root`。顶层 `Events` 只保存 `eventID`、`eventIDTime`、`C1Time/C1Amp`、`C2Time/C2Amp`、`C3Time/C3Amp` 和 `referenceTime`；三路时间均已减去 `eventIDTime`，`referenceTime` 是由三组时间差解出单路分辨率后得到的逆方差加权平均。文件还保存三张带核心高斯拟合的 `hDeltaT_C1_C2/C1_C3/C2_C3` 和一张 `hReferenceTime`，不写其他 tree 或目录。必要时可覆盖 `csvDirectory`、`outputFile`、`cfdFraction`、`minPulseAmplitude`、`histogramBins`、`maxWaveformFiles`、`eventIDTimeWindowNs`、`eventIDMedianFilterSamples` 和 `signalTimeWindowNs`。
