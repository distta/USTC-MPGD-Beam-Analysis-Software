...
//
// The horizontal axis is the absolute value of voltage2 ("High voltage").
...
// ======================== USER CONFIGURATION ========================
const std::string dataDirectory = "data/TrackerTimeRes";
const std::string detectorName = "#muRGroove";
...
const std::array<ScanSetting, 11> kScanSettings = {{
    {-180, 400, 209},
    {-180, 390, 211},
    {-180, 380, 213},
    {-180, 370, 215},
...
// One-based (column, row) pad bins. These bins remain near zero
...
EfficiencyResult ReadEfficiency(const std::string &path, int dutID) {
  ...
  // Current DUTInfo files store the denominator and the configured 2D
  // envelope efficiency as histograms inside canvases. Reconstructing the
  // binomial counts from these objects preserves the analysis definition
  // while allowing this scan macro to mask individual bad pads.
  const std::string efficiencyDirectory =
      "DUT_" + std::to_string(dutID) + "/Efficiency/";
  auto *eventCanvas = dynamic_cast<TCanvas *>(
      file.Get((efficiencyDirectory + "cEventCount").c_str()));
  auto *efficiencyCanvas = dynamic_cast<TCanvas *>(
      file.Get((efficiencyDirectory + "cEffMap2D").c_str()));
  auto *eventCount =
      eventCanvas
          ? dynamic_cast<TH2D *>(
                eventCanvas->GetListOfPrimitives()->FindObject("hEventCount"))
          : nullptr;
  auto *efficiencyMap =
      efficiencyCanvas
          ? dynamic_cast<TH2D *>(
                efficiencyCanvas->GetListOfPrimitives()->FindObject("hEff2D"))
          : nullptr;
  if (eventCount && efficiencyMap &&
      eventCount->GetNbinsX() == efficiencyMap->GetNbinsX() &&
      eventCount->GetNbinsY() == efficiencyMap->GetNbinsY()) {
    for (int xBin = 1; xBin <= eventCount->GetNbinsX(); ++xBin) {
      for (int yBin = 1; yBin <= eventCount->GetNbinsY(); ++yBin) {
        const long long total =
            std::llround(eventCount->GetBinContent(xBin, yBin));
        if (total <= 0)
          continue;
        ...
        const double efficiency =
            efficiencyMap->GetBinContent(xBin, yBin);
        if (!std::isfinite(efficiency) || efficiency < 0.0 ||
            efficiency > 1.0) {
          continue;
        }
        result.total += total;
        result.passed += std::llround(efficiency * total);
      }
    }
    if (result.total > 0) {
      result.value =
          static_cast<double>(result.passed) / result.total;
  ...
  // Backward-compatible fallback for the former pad-DUT output layout.
  auto *tree = dynamic_cast<TTree *>(file.Get("PadDUTTree"));
  auto *map = dynamic_cast<TH2D *>(file.Get(
      ("DUT_" + std::to_string(dutID) + "/Efficiency/hEfficiency2D").c_str()));
  if (!tree || !map)
    return result;
  ...
  for (Long64_t entry = 0; entry < tree->GetEntries(); ++entry) {
    ...
    if (xBin < 1 || xBin > xAxis->GetNbins() || yBin < 1 ||
        yBin > yAxis->GetNbins()) {
      continue;
    }
    if (kBadPads.count({xBin, yBin}) != 0) {
  ...
  const double lower =
      TEfficiency::ClopperPearson(result.total, result.passed, 0.683, false);
  const double upper =
      TEfficiency::ClopperPearson(result.total, result.passed, 0.683, true);
  result.errorLow = result.value - lower;
  result.errorHigh = upper - result.value;
  result.valid = true;
  return result;
...
TimingResult ReadTiming(const std::string &path, int dutID) {
  ...
  if (results && results->GetBranch("analysis") &&
      results->GetBranch("object") && results->GetBranch("detectorA") &&
      results->GetBranch("entries") && results->GetBranch("resolutionNs")) {
    ...
    for (Long64_t entry = 0; entry < results->GetEntries(); ++entry) {
      ...
      if (!analysis || !object || *analysis != "without_external_t0" ||
...
void PlotLHCb75umRunScan(const char *baseDirectoryArgument = "",
  ...
  const std::string baseDirectory =
      baseDirectoryArgument && baseDirectoryArgument[0] != '\0'
          ...
          : dataDirectory;
  const int dutID = dutIDArgument >= 0 ? dutIDArgument : configuredDUTID;
  gStyle->SetOptStat(0);
  gStyle->SetTitleBorderSize(0);

  std::vector<ScanSetting> settings(kScanSettings.begin(), kScanSettings.end());
  std::sort(settings.begin(), settings.end(),
            [](const ScanSetting &first, const ScanSetting &second) {
              return std::abs(first.voltage2) < std::abs(second.voltage2);
            });
  ...
  for (const ScanSetting &setting : settings) {
    ...
    EfficiencyResult efficiency =
        ReadEfficiency(RunFile(baseDirectory, run, "DUTInfo.root"), dutID);
    if (!efficiency.valid) {
      efficiency =
          ReadEfficiency(RunFile(baseDirectory, run, "PadDUTInfo.root"), dutID);
    }
    const TimingResult timing =
        ReadTiming(RunFile(baseDirectory, run, "TimeResolution.root"), dutID);

    if (efficiency.valid && timing.valid) {
      const int point = efficiencyCurve.GetN();
      efficiencyCurve.SetPoint(point, voltage, 100.0 * efficiency.value);
      efficiencyCurve.SetPointError(point, 0.0, 0.0,
                                    100.0 * efficiency.errorLow,
    ...
    std::cout << "run " << std::setw(3) << run << "  V1=" << std::setw(5)
              << setting.voltage1 << " V  V2=" << std::setw(5)
              << setting.voltage2 << " V";
    if (efficiency.valid) {
      std::cout << "  efficiency=" << std::fixed << std::setprecision(3)
                << 100.0 * efficiency.value << "%" << " (" << efficiency.passed
    ...
    if (timing.valid) {
      std::cout << "  time=" << std::fixed << std::setprecision(3)
                << timing.resolution << " +/- " << timing.error << " ns";
      if (timing.timeWalkApplied)
        std::cout << " [time-walk corrected]";
    } else {
  ...
  const double voltageSpan = std::max(1.0, maximumVoltage - minimumVoltage);
  ...
  const double timingMinimum = std::max(0.0, minimumTiming - 0.20 * timingSpan);
  const double timingMaximum = maximumTiming + 0.30 * timingSpan;
  const double efficiencyMinimum = std::max(0.0, minimumEfficiency - 3.0);
  ...
  for (int point = 0; point < timingCurve.GetN(); ++point) {
    ...
    const double scaled =
        efficiencyMinimum + (timing - timingMinimum) /
                                (timingMaximum - timingMinimum) *
                                (efficiencyMaximum - efficiencyMinimum);
    const double scaledError = timingCurve.GetErrorY(point) /
                               (timingMaximum - timingMinimum) *
                               (efficiencyMaximum - efficiencyMinimum);
  ...
  TGaxis timingAxis(voltageMaximum, efficiencyMinimum, voltageMaximum,
                    efficiencyMaximum, timingMinimum, timingMaximum, 510, "+L");
  ...
  const std::string outputDirectory =
      outputDirectoryArgument && outputDirectoryArgument[0] != '\0'
          ...
          : (configuredOutputDirectory == "auto" ? resultDirectory
